...
public class ProceduralMapGenerator3D : ProceduralMapGenerator
{
    [Header("3D Generation S    ettings")]
    public bool generate3D = true;
    public List<StructureData> structureDatabase = new List<StructureData>();
    ...
    private WFC3DExtension wfc3DExtension;
    ...
    private IEnumerator GenerateMap3DCoroutine()
    {
        ...
        UpdateStatusText("Step 1/4: Generating 2D base map...");
        ...
        if (generate3D)
        {
            UpdateStatusText("Step 2/4: Generating height map...");
            yield return StartCoroutine(GenerateHeightMapCoroutine());
        }
        
        UpdateStatusText("Step 3/4: Spawning tiles with height...");
        yield return StartCoroutine(SpawnTilesWithHeight());
        

        if (generate3D && generateStructures)
        {
            UpdateStatusText("Step 4/4: Placing structures...");
    ...
    {
        wfc3DExtension = new WFC3DExtension(currentGrid, structureDatabase, biomeType, seed);
